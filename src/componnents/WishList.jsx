...
export default function wish() {
 ...
 let [open,setOpen] = useState (false)
  ...
  let {data,isError,error,isLoading} = useQuerywish()
  let {mutate,data:deleteddata,isPending} = usedeletwish(deleteItem) 

  let {mutate:updatemutate, data:updateData,isPending:updatePending} = usedeletwish(updateItem) 

  let {setwishNum} = useContext(wishNums)

  setwishNum (data?.data?.numOfwishItems)
  ...
  return (
    <div className='container' >
     <div>
      ...
      <h1  className='my-4 font-bold text-[2rem]'> Total wish item {data?.data?.numOfwishItems}</h1> 
      <h1 className='my-4 font-bold text-[2rem] text-green-color ' > Total price {data?.data?.data?.totalwishPrice} EGP </h1>
...
{data?.data?.data?.products?.map(prod=>
    ...
    {open&&  <Payement wishId={data?.data?.wishId}/>}
